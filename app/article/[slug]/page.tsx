...
const client = createClient({
  ...
  useCdn: false,
  // ✅ الـ Token الصحيح مأخوذ من صورتك
...
export default function ArticlePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (!data) {
          setError("المقال موجود فـ Sanity ولكن الـ Slug مكيطابقش مع الرابط.");
        } else {
          setArticle(data);
        }
      } catch (err: any) {
        console.error("❌ Sanity Error:", err);
        setError("مشكل فـ الاتصال: " + err.message);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) return (
    <div style={{padding: "100px", textAlign: "center", fontFamily: "sans-serif"}}>
      ...
      <p>إيلا بقا هاد الميساج، كاين مشكل فـ الـ Token ولا الـ CORS.</p>
    </div>
  );
